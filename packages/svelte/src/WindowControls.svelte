...
{:else if resolvedPlatform === "gnome"}
  <div class={cn(styles.gnome.container, customClass)}>
    <button aria-label="Minimize" onclick={() => controls?.minimize()} class={cn(styles.baseButton, styles.gnome.button)}>
    ...
    <button aria-label="Maximize" onclick={() => controls?.maximize()} class={cn(styles.baseButton, styles.gnome.button)}>
      {#if isMaximized}
    ...
    <button aria-label="Close" onclick={() => controls?.close()} class={cn(styles.baseButton, styles.gnome.button)}>
      <svg class="h-2 w-2" width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{:else}
  <div class={cn(styles.windows.container, customClass)}>
    <button aria-label="Minimize" onclick={() => controls?.minimize()} class={cn(styles.baseButton, styles.windows.button)}>
      <svg width="10" height="1" viewBox="0 0 10 1" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d={icons.windows.minimize.path} fill="currentColor" fill-opacity="0.8956" />
      </svg>
    </button>
    <button aria-label="Maximize" onclick={() => controls?.maximize()} class={cn(styles.baseButton, styles.windows.button)}>
      {#if isMaximized}
    ...
    <button aria-label="Close" onclick={() => controls?.close()} class={cn(styles.baseButton, styles.windows.closeButton)}>
      <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d={icons.windows.close.path} fill="currentColor" fill-opacity="0.8956" />
      </svg>
    </button>
